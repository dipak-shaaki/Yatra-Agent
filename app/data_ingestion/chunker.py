...
def chunk_document(file_path: str | Path) -> list[dict[str, Any]]:
    ...
    intro_text = body[:first_header_match.start()].strip() if first_header_match else body.strip()
    if intro_text:
        chunks.append({
            "id": f"{file_path.stem}__overview",
            "text": f"{dest_name} — Overview:\n{intro_text}",
            "metadata": {**metadata_base, "section_title": "Overview", "source_file": file_path.name},
        })
    ...
    for i, header in enumerate(headers):
        ...
        chunks.append({
            ...
            "metadata": {**metadata_base, "section_title": header, "source_file": file_path.name},
        })
